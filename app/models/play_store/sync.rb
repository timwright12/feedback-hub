...
module PlayStore
  class Sync
    ...
    def sync_reviews(app_id:, start_date: Date.today, end_date: Date.today, force_sync: false)
      (start_date.to_datetime..end_date.to_datetime).each do |date|
        if force_sync || should_sync_date?(date)
          ...
          runner = Runner.create(source: :play_store, start_date: date.beginning_of_day, end_date: date.end_of_day)
    ...
    def should_sync_date?(date)
      !Runner.synced_date?(date:, source: :play_store)
    end

    # rubocop:disable Metrics/AbcSize
    def sync_submissions(app_id:, date: Date.today)
      Rails.logger.info 'PlayStoreSyncJob: syncing submissions.'
      existing_reviews = PlayStore::Review.includes(:author)
                                          .where(play_store_updated_at: date.beginning_of_day..date.end_of_day)
                                          .order(:play_store_updated_at)
